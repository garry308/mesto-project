...
let user = new UserInfo(userInputSelectors, api);
...
export function postImage(evt) {
  ...
  user.setUserAvatar(imageInput.value);
  evt.submitter.classList.remove('popup__save-button_active');
  imageInput.value = "";
...
(function () {
  Promise.all([user.getUserInfo(), api.getInitialCards()])
    .then(([profile, cards]) => {
      mainPageName.setAttribute('id', profile.id);
      user.setUserInfo(profile);
      user.setUserAvatar(profile.avatar);
      profileNameInput.value = profile.name;
      profileBioInput.value = profile.about;
      const cardsSection = new Section({
          items: cards,
          renderer: (card) => {
            const newCard = new Card(card, '#card', api);
